...
DetectorMulti::DetectorMulti (Data &data)
{
  _data = &data;
...
double DetectorMulti::getSE (double x, double a, double b)
{
  if (b != 0)
...
double DetectorMulti::getSE (int x, int y, double a, double b)
{
  return getSE (_edgeCount->coeff (x, y), a, b);
}


void DetectorMulti::execute ()
{
  fillTable ();
...
void DetectorMulti::initH (int h)
{
  for (int i = 0; i < _N; i++) {
    for (int j = 0; j < _N; j++) {
      for (int k = 0; k < _N; k++) {
...
void DetectorMulti::backTrace (int k, int h, bool add)
{
...
void DetectorMulti::multiSplit (int start, int end, int k, int h, int parentEnd, bool add)
{
  if (add)
